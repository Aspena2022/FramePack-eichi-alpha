...
def optimize_state_dict_with_fp8_on_the_fly(
    ...
    if exp_bits == 4 and mantissa_bits == 3:
        ...
        raise ValueError(f"サポートされていないFP8形式: E{0}M{1}".format(exp_bits, mantissa_bits))
    ...
    print(f"最適化された線形レイヤー数: {0}".format(optimized_count))
    return state_dict
    ...
    if use_scaled_mm:
        ...
        if weight_dtype != torch.float8_e4m3fn:
            if not FP8_E4M3_WARNING_SHOWN:
                print(f"警告: scaled_mmはFP8 E4M3FN形式を必要としますが、{weight_dtype}が検出されました。通常方式にフォールバックします。".format(weight_dtype=weight_dtype))
        ...
        if x.ndim != 3:
            if not FP8_DIMENSIONS_WARNING_SHOWN:
                print(f"警告: scaled_mmは3次元入力が必要ですが、{0}次元が検出されました。通常方式にフォールバックします。".format(x.ndim))
...
def apply_fp8_monkey_patch(model, optimized_state_dict, use_scaled_mm=False):
    ...
    print(f"モンキーパッチ適用済みの線形レイヤー数: {0}".format(patched_count))
    # モデルにFP8適用済みフラグを設定
...
def check_fp8_support():
    ...
    if has_e4m3 and has_e5m2:
        if has_scaled_mm:
            print(f"RTX 40シリーズのGPUでFP8の高速化が可能です")
    else:
        print(f"警告: FP8サポートが検出されませんでした。PyTorch 2.1以上が必要です")
