...
def unified_keyframe_change_handler(keyframe_idx, img, mode, length, enable_copy=True, dynamic_sections=None):
    ...
    if mode != MODE_TYPE_LOOP:
        enable_copy = False
        print(f"[keyframe_handler] 通常モードでコピー機能を強制的に無効化")
    ...
    if dynamic_sections is not None:
        ...
        print(f"[keyframe_handler] 動的に計算されたセクション数を使用: {sections}")
    else:
        # video_mode_settings.pyのデフォルト値を取得
        sections = get_total_sections(length)
        print(f"[keyframe_handler] 設定からのデフォルトセクション数を使用: {sections}")
    print(f"[keyframe_handler] 動画長 {length} のセクション数: {sections}")
    ...
    if keyframe_idx == 0:
        ...
        print(f"[keyframe_handler] 赤枠(セクション0)から偶数番号へのコピーターゲット: {targets} (動的セクション数: {sections})")
        # コピー先が空の場合はログに出力
        if not targets:
            print(f"[keyframe_handler] 赤枠(セクション0)からのコピー先がありません。セクション数が少ない可能性があります。")
    
    # 青枠（セクション1）からのコピー先を計算
    elif keyframe_idx == 1:
        # 3から始まる奇数番号を、実際のセクション数の範囲内でリストに追加
        targets = [i for i in range(3, sections) if i % 2 == 1]
        print(f"[keyframe_handler] 青枠(セクション1)から奇数番号へのコピーターゲット: {targets} (動的セクション数: {sections})")
        # コピー先が空の場合はログに出力
        if not targets:
            print(f"[keyframe_handler] 青枠(セクション1)からのコピー先がありません。セクション数が少ない可能性があります。")
    ...
    for i in range(keyframe_idx + 1, max_keyframes):
        ...
        if i in targets:
            ...
            print(f"[keyframe_handler] セクション{i}へ画像をコピーします")
